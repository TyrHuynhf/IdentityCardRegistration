...
export class CardContract extends Contract {
    ...
    public async createCardRegistration (ctx: Context, cardAsString: string) {
        const card: Card = JSON.parse(cardAsString);
    ...
    //update Card
    public async updateCard (ctx: Context, cardID: string, payload: string){
        try{
            ...
            const card = {...currentCard, ...payloadUpdate};
            card.modifyTime = new Date(ctx.stub.getTxTimestamp().seconds *1000).toString();
    ...
    //getCard
    public async getCard(ctx: Context, cardID: string){
        try{
            const cardAsBytes = await ctx.stub.getState(cardID);
    ...
    public async queryAllCards(ctx: Context) {
    ...
    //accept Registration
    public async acceptRegistration (ctx: Context,  cardID: string, cardNumber: string ){
        try{
            const cardAsBytes = await ctx.stub.getState(cardID);
            const card = JSON.parse(cardAsBytes.toString());
    ...
    public async rejectRegistration (ctx: Context, cardID: string){
        try{
            const cardAsBytes = await ctx.stub.getState(cardID);
            const card = JSON.parse(cardAsBytes.toString());
            card.modifyTime = new Date(ctx.stub.getTxTimestamp().seconds *1000).toString();
    ...
    public async approveRegistration (ctx: Context, cardID: string){
        try{
            const cardAsBytes = await ctx.stub.getState(cardID);
            const card = JSON.parse(cardAsBytes.toString());
            card.modifyTime = new Date(ctx.stub.getTxTimestamp().seconds *1000).toString();
    ...
    //deleteCardByiD
    public async deleteCardByID(ctx: Context, cardID: string) {
        ctx.stub.deleteState(cardID);
        return {TxID: ctx.stub.getTxID}
    }
    public async deleteCity(ctx: Context, cityID: string) {
        await ctx.stub.deleteState(cityID);
    }

    //queryCardByOwnerPhoneNumber
    public async queryCardByOwnerPhoneNumber(ctx: Context, phoneNumber: string){
        try{
            const queryString : any ={};
            queryString.selector = {
                docType : "card",
    ...
    public async queryAllPendingRegistration(ctx: Context){
        try{
            const queryString: any ={};
            queryString.selector={
    ...
    public async queryResult (ctx: Context, queryString: string){
    ...
    //add city
    public async addCity (ctx: Context, ...cityAsString: string[]){
        try{
            const city: City = {
                id: cityAsString[0],
