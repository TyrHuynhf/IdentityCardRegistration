    ...
    >
      <template v-slot:top>
        <v-toolbar
          flat
        >
          <v-toolbar-title>Đăng ký đã bị loại bỏ</v-toolbar-title>
          <v-divider
            class="mx-4"
            inset
            vertical
...
export default {
  data(){
    return{
      ...
      {text:'Lý do bị loại bỏ', value:'rejectReason',sortable: false},
      ],
      search:'',
      test:[],
      statusColor : {
  ...
  created() {
    axios.get('http://localhost:3000/cards/rejectedCards/test')
    .then(res=>{
