    ...
    >
      <template v-slot:top>
        <v-toolbar
          flat
        >
          <v-toolbar-title>Đăng ký đang chờ xử lý</v-toolbar-title>
          <v-divider
            class="mx-4"
            inset
            vertical
...
<!--      </template>-->
      ...
      <template  class='text-center' #item.invitation="{ item }">
        <span> {{item.invitationDate }} {{item.inviteTime}} </span>
      </template>
...
export default {
  data(){
    return{
      ...
      {text:'Ngày hẹn', value:'invitation',sortable: false},
      {text: 'Chân dung', value: 'pictureOf', sortable: false},
      {text: 'Vân tay', value: 'fingerPrint', sortable: false},
      // { text: 'Hành động', value: 'actions', sortable: false },
      { text: 'Hoàn thành', value: 'completeCard', sortable: false },
      ],
      search:'',
      test:[],
      statusColor : {
    ...
    mungedCard(){
      return this.test.map((v)=>{
        return {
          ...
          sortable_invitation: this.formatDateWithoutHour(v.invitationDate)
        }
      })
    }
  },
  ...
  created() {
    axios.get('http://localhost:3000/cards/pendingCards/test')
    .then(res=>{
