...
export default {
  ...
  data() {
    return {
      ...
      passwordRules: [
        v => !!v || 'Bắt buộc',
        v => (v && v.length <= 10) || 'mật khẩu phải ít hơn 10 ký tự'
      ],
      nameRules: [
        v => !!v || 'Bắt buộc',
        v => (v && v.length <= 56) || 'Tên phải ít hơn 56 ký tự'
      ],
      phoneRules: [
        v => !!v || 'Bắt buộc',
  ...
  methods:{
    updateUser(){
      axios.put('http://localhost:3000/users/'+ this.account.id, this.account, policeConfig )
      .then(res => {
        console.log(res.data)
        Swal.fire({
          tile: 'Cập nhật tài khoản thành công',
          icon:'success'
        })
        //this.$emit('refresh')
        this.$router.go()
